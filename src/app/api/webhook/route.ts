...
export async function POST(req: NextRequest) {
  ...
  if (eventType === "call.session_started") {
    ...
    if (!updatedMeeting) {
      return NextResponse.json({ error: "Duplicate webhook" }, { status: 200 });
    }
    ...
    if (userId !== existingAgent.id) {
      ...
      .where(
        and(
          ...
          or(
            isNull(meetings.lastProcessedMessageId),          
            not(eq(meetings.lastProcessedMessageId, messageId)) 
          )
        )
      )
      .returning();
